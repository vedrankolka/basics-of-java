...
public class SmartScriptLexer {
	private static final char TAG_BEGINING = '{';
	...
	private static final char[] VALID_ESCAPE_FOR_TEXT = {ESCAPE_CHAR, TAG_BEGINING};
	private static final char[] VALID_ESCAPE_FOR_STRING = {ESCAPE_CHAR, '"', 'n', 'r', 't'};
...
//private helper methods------------------------------------------------------------------------
	...
	private Token extractBasicToken() {
		...
		throw new LexerException("Unexpected syntax in BASIC state.");
	}
	...
	private Token extractTagToken() {
		...
		throw new LexerException("Unexpected syntax in TAG state.");
	}
	/**
		...
		while(currentIndex<data.length && !checkNextCharacter(currentIndex, '"')) {
			...
			if(checkNextCharacter(currentIndex, ESCAPE_CHAR)) {
				...
				default:
					...
					throw new LexerException("Unexpected escape sequence.");
				}
				currentIndex++;
			} else {
	...
	private void checkEscapeSequence(char ...validCharacters) {
		if(!checkNextCharacter(currentIndex+1, validCharacters)) {
			throw new LexerException("Invalid use of escape character.");
		}
	}
