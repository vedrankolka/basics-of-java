...
public class RayCasterParallel {
	...
	public static void main(String[] args) {
		RayTracerViewer.show(new MyRayTracerProducer(),
				new Point3D(10,0,0),
				new Point3D(0,0,0),
				new Point3D(0,0,10),
				20, 20);
	...
	/**
	 * An implementation of a IRayProducer that uses a ForkJoinPool for parallelization
	 * of producing the rgb components for the screen.
	 * @author Vedran Kolka
	 *
	 */
	public static class MyRayTracerProducer implements IRayTracerProducer {
		...
		public void produce(Point3D eye, Point3D view, Point3D viewUp,
			...
			pool.invoke(new TracerJob(eye, xAxis, yAxis, zAxis, corner, scene,
					horizontal, vertical, width, height, 0, height, cancel, red, green, blue));
			pool.shutdown();
	 ...
	 * The job is split recursively and its part of the screen is defined by the THRESHOLD.
	 * @author Vedran Kolka
	 *
	 */
	public static class TracerJob extends RecursiveAction {
		 ...
		 */
		public TracerJob(Point3D eye, Point3D xAxis, Point3D yAxis, Point3D zAxis, Point3D corner, Scene scene, double horizontal,
				double vertical, int width, int height, int yMin, int yMax, AtomicBoolean cancel, short[] red,
		...
		/**
		 * Splits the job until the job is small enough to compute directly.
		 */
		@Override
		public void compute() {
			...
			int yMin2 = yMax1;
			invokeAll(
				new TracerJob(eye, xAxis, yAxis, zAxis, corner, scene, horizontal, vertical, width, height,
						yMin, yMax1, cancel, red, green, blue),
				new TracerJob(eye, xAxis, yAxis, zAxis, corner, scene, horizontal, vertical, width, height,
						yMin2, yMax, cancel, red, green, blue)
			);
			
		}
		
		/**
		 * The method that actually computes the needed rgb components.
		 */
